...
package ch.obermuhlner.libgdx.planetbrowser.model;
...
public class MeshBuilder implements MeshPartBuilder {
	...
	private final Matrix3 tangentTransform = new Matrix3();
	...
	public void setVertexTransform (Matrix4 transform) {
		if ((vertexTransformationEnabled = (transform != null)) == true) {
			...
			tangentTransform.idt(); // FIXME FNORD
		} else {
			positionTransform.idt();
			normalTransform.idt();
			tangentTransform.idt();
		}
	}
	...
	private static final Vector3 DEFAULT_TANGENT = new Vector3(0, 0, 1);
	...
	private final static void transformTangent (final float[] values, final int offset, final int size, Matrix3 transform) {
		// FIXME FNORD
		transformNormal(values, offset, size, transform);
	}
	...
	private final void addVertex (final float[] values, final int offset) {
		...
		if (vertexTransformationEnabled) {
			...
			if (tanOffset >= 0) transformTangent(vertices.items, o + tanOffset, 3, tangentTransform);
		}
	...
	@Override
	public short vertex (Vector3 pos, Vector3 nor, Vector3 tan, Color col, Vector2 uv) {
		...
		if (tanOffset >= 0) {
			if (tan == null) tan = DEFAULT_TANGENT;
	...
	public short vertex (final VertexInfo info) {
		return vertex(info.hasPosition ? info.position : null, info.hasNormal ? info.normal : null, info.hasTangent ? info.tangent : null, info.hasColor ? info.color
	...
	public void sphere (final Matrix4 transform, float width, float height, float depth, int divisionsU, int divisionsV,
		...
		for (int iv = 0; iv <= divisionsV; iv++) {
			...
			for (int iu = 0; iu <= divisionsU; iu++) {
				...
				curr1.normal.set(curr1.position).nor();
				//curr1.tangent.set(new Vector3(0, 0, 1)); // FIXME FNORD
				curr1.tangent.set(curr1.normal).crs(Vector3.Y).scl(-1); // FIXME FNORD
				curr1.uv.set(u, v);
